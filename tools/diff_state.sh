...
{
  echo "To install:"; sed 's/^/  - /' "$OUT_DIR/_brew_to_install.txt"; echo ""
  echo "Not in desired (extras):"; sed 's/^/  - /' "$OUT_DIR/_brew_extras.txt"
} >> "$REPORT"
...
{
  echo "To install:"; sed 's/^/  - /' "$OUT_DIR/_casks_to_install.txt"; echo ""
  echo "Not in desired (extras):"; sed 's/^/  - /' "$OUT_DIR/_casks_extras.txt"
} >> "$REPORT"
...
{
  echo "To install (by id):"; sed 's/^/  - /' "$OUT_DIR/_mas_to_install.txt"; echo ""
  echo "Not in desired (extras, by id):"; sed 's/^/  - /' "$OUT_DIR/_mas_extras.txt"
} >> "$REPORT"
...
if [[ -f "$DES_DEF" ]]; then
  {
    ...
    while IFS=$'\t' read -r domain key type desired_value; do
      ...
      if [[ "$norm_cur" != "$norm_des" ]]; then
        printf "  - %s %s: current=%s -> desired=%s\n" "$domain" "$key" "$current_value" "$desired_value"
      fi
    done < "$DES_DEF"
  } >> "$REPORT"
else
